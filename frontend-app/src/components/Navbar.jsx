...
import { Bell, Home, Map, Menu, Recycle, User, X, ArrowRight, Plus, Settings, LogOut, ChevronDown } from 'lucide-react';
...
const menuVariants = {
  ...
  visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.22, ease: 'easeOut', staggerChildren: 0.04 } },
...
export default function Navbar() {
  ...
  const userName = user?.username || 'Donatur';
  const userPhoto = user?.foto || user?.avatar || user?.profile_picture || null;
              ...
              {navLinks.map(({ name, path }) => {
                ...
                return (
                  <Link key={name} to={path} className={`relative rounded-full px-4 py-2 text-[12px] font-semibold transition-colors duration-200 ${isActive ? 'text-[#6d5b00]' : 'text-gray-500 hover:text-gray-900'}`}>
                    {isActive && <motion.span layoutId="desktop-navbar-active" transition={{ type: 'spring', stiffness: 420, damping: 32 }} className="absolute inset-0 rounded-full bg-[#FFD500]" />}
            ...
            <div className="ml-auto hidden items-center gap-3 md:flex">
              ...
              {isAuthenticated ? (
                <div ref={profileRef} className="relative">
                  <button type="button" onClick={() => setIsProfileOpen(value => !value)} className={`group flex items-center gap-2 rounded-full py-1 pl-1 pr-2 transition ${isProfileOpen ? 'bg-gray-100' : 'hover:bg-gray-50'}`}>
                    ...
                    <div className="max-w-[105px] text-left leading-none">
                  ...
                  <AnimatePresence>
                    {isProfileOpen && (
                      <motion.div initial={{ opacity: 0, y: -6, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -5, scale: .97 }} transition={{ duration: .18, ease: 'easeOut' }} className="absolute right-0 top-[calc(100%+10px)] w-56 origin-top-right overflow-hidden rounded-2xl border border-gray-100 bg-white p-2 shadow-[0_18px_45px_rgba(15,23,42,.12)]">
                        ...
                        <button type="button" onClick={logout} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-semibold text-red-500 transition hover:bg-red-50">
              ...
              <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white p-3 shadow-[0_20px_50px_rgba(0,0,0,.12)]">
                  ...
                  {navLinks.map(({ name, path, icon: Icon }) => {
                    ...
                    return (
                      <motion.div key={name} variants={itemVariants}>
                        <Link to={path} className={`flex items-center gap-3 rounded-xl px-3 py-3 transition ${isActive ? 'bg-[#FFFBEA] text-[#806b00]' : 'text-gray-600 hover:bg-gray-50'}`}>
                          <span className={`grid h-9 w-9 place-items-center rounded-lg ${isActive ? 'bg-[#FFD500] text-gray-950' : 'bg-gray-100 text-gray-500'}`}>
                            <Icon size={17} />
                          </span>
                ...
                {isAuthenticated ? (
                  <div className="rounded-2xl border border-gray-100 bg-gray-50 p-3">
                    ...
                    <button type="button" onClick={logout} className="mt-2 flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-red-50 text-[11px] font-bold text-red-500 transition hover:bg-red-100">
                      <LogOut size={14} />
                      Keluar
                    </button>
                  </div>
                ) : (
                  <Link to="/login" className="flex h-11 items-center justify-center gap-2 rounded-xl bg-[#FFD500] text-xs font-black text-gray-950 shadow-sm">
                    Guest Start
                    <ArrowRight size={15} />
                  </Link>
                )}
